...
public class Entrance {
     ...
     *    1. client和server进行连接，若某时间间隔(如120s)内没有任何交互，则client断开连接。若有交互，则保持连接，直至下一个120s内没有任何交互时再断开连接。
     *    2. 每积累1000条记录发送一次请求。
     *    3. 若在300秒内，积累的记录不足1000条，也会强制性的向服务端发送一次记录。(无论是0条、1条还是999条数据)
     *       (Tips: 这里的300秒是一个动态的概念。如果在300秒之内发送了一次数据请求，则自动在当前时间点向后延长300秒，将未来的那个时刻记录成"待强制发送请求的时刻")
     *    也即，每次客户端和服务端建立连接并发送请求，都会重置定时任务。 (这里可以想到ScheduleExecutorPool 定时线程池)
     *    优点:
     *          1. 不用长时间连接server，server压力不大。
     *          2. 若有数据则能相对较快的响应client的请求(重新建立连接并向server发出请求肯定要花费一定的时间，所以我用的是"相对较快")
     *    缺点:
     *          1. 数据的即时性不是那么高。(前面的120s, 300s, 1000条记录 都是解决即时性低的手段)
     */
}
